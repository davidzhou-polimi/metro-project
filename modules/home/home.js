...
function setupHome() {
    ...
    container.html("");
    document.title = "World Metro";

    // Layout principale
    let wrapper = createDiv().parent(container);
    
    createElement("h1", "Seleziona una città").parent(wrapper)
        .class("text-4xl font-black text-neutral-800 mb-8 text-center tracking-tight");
    ...
    if (typeof db !== 'undefined' && db.cities) {
        ...
        for (let city of sortedCities) {
            let card = createDiv().parent(grid)
                .class("bg-white p-6 rounded-xl shadow-sm border border-neutral-200 hover:shadow-md hover:border-neutral-300 transition-all cursor-pointer group relative overflow-hidden");
            ...
            createElement("h3", city.name).parent(card)
                .class("text-xl font-bold text-neutral-700 group-hover:text-neutral-600 mb-1");
            createSpan(city.country).parent(card)
                .class("text-xs font-bold text-neutral-400 uppercase tracking-wider");
            createDiv().parent(card).class("absolute bottom-0 left-0 h-1 bg-neutral-500 w-0 group-hover:w-full transition-all duration-300");
        }
    } else {
        createP("Attendi caricamento dati...").parent(wrapper).class("text-center text-neutral-400");
    }
}
