    ...
    if (typeof db !== 'undefined' && db.cities) {
        ...
        for (let city of sortedCities) {
            let card = createDiv().parent(grid)
                .class("bg-white p-6 rounded-xl shadow-sm border border-slate-200 hover:shadow-md hover:border-neutral-300 transition-all cursor-pointer group relative overflow-hidden");
            ...
            createElement("h3", city.name).parent(card)
                .class("text-xl font-bold text-slate-700 group-hover:text-neutral-600 mb-1");
